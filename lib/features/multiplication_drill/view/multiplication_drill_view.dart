...
import '../../../providers/multiplication_drill_provider.dart';
import '../../shared/widgets/timer_widget.dart';
import '../../shared/widgets/answer_input_widget.dart';

class MultiplicationDrillView extends StatefulWidget {
  const MultiplicationDrillView({super.key});

  @override
  State<MultiplicationDrillView> createState() =>
      _MultiplicationDrillViewState();
}

class _MultiplicationDrillViewState extends State<MultiplicationDrillView> {
  @override
  void initState() {
    super.initState();
    // Start the session when the view is initialized
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<MultiplicationDrillProvider>().startSession();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Multiplication Drill'),
        elevation: 0,
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: () =>
                context.read<MultiplicationDrillProvider>().resetSession(),
            tooltip: 'Reset Session',
          ),
        ],
      ),
      body: Consumer<MultiplicationDrillProvider>(
  ...
  Widget _buildScoreDisplay(
      BuildContext context, MultiplicationDrillProvider viewModel) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildScoreItem(context, 'Questions',
              '${viewModel.session.totalQuestions}', AppTheme.primaryGreen),
          _buildScoreItem(
              context,
              'Accuracy',
      ...
      child: Column(
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _buildNumberBox(context, question.operand1, AppTheme.primaryBlue),
              ...
              const SizedBox(width: 16),
              _buildNumberBox(context, question.operand2, AppTheme.primaryBlue),
              const SizedBox(width: 16),
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
